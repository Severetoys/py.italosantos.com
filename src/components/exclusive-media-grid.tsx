...
export function useExclusiveMedia() {
  ...
  useEffect(() => {
    const loadMedia = async () => {
      ...
      try {
        // Por enquanto, vamos buscar diretamente do endpoint admin para testar
        ...
        if (response.ok) {
          ...
          const convertedMedia: MediaItem[] = data.content?.map((item: any) => {
          ...
          console.error('Erro ao carregar conteúdo exclusivo:', response.statusText);
          // Em caso de erro, usar dados de fallback vazios
          setMedia([]);
        }
      } catch (error) {
