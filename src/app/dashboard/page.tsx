        ...
        <Card className="flex flex-col w-full p-6 bg-card/50 backdrop-blur-sm border-primary/20 hover:border-primary hover:shadow-neon-red-light transition-all duration-300">
          <CardContent className="flex flex-col items-start text-left p-0 flex-grow gap-4">
            ...
            {review.reply && (
             ...
             <Button variant="link" className="p-0 h-auto text-xs text-muted-foreground hover:text-white">Responder</Button>
          </CardContent>
        </Card>
    );
  }
        ...
        <CardHeader>
            ...
            <CardDescription>
              {hasActiveSubscription 
                ? `Plano ${plan?.name || 'Ativo'} - ${subscription ? `Expira em ${new Date(subscription.endDate).toLocaleDateString('pt-BR')}` : 'Ativo'}`
    ...
    <main className="flex flex-1 w-full flex-col items-center justify-start p-4 bg-background gap-8">
       <div className="w-full max-w-4xl space-y-8">
            ...
            <div className="w-full">
                <h2 className="text-3xl font-bold text-center mb-8 text-white">Comentários</h2>
