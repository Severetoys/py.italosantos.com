  ...
  return (
      <Card className="w-full max-w-6xl animate-in fade-in-0 zoom-in-95 duration-500 shadow-neon-red-strong border-primary/50 bg-card/90 backdrop-blur-xl">
        <CardHeader className="flex-row items-center justify-between border-b border-primary/20 pb-4">
          <CardTitle className="text-3xl text-white text-center flex-1">
            Adult Store
          </CardTitle>
          <div className="w-[150px]">
             <PayPalButtons style={{ layout: 'horizontal', color: 'gold', shape: 'rect', label: 'buynow' }} disabled />
          </div>
        </CardHeader>
                           ...
                           <PayPalButtons
                                ...
                                onApprove={(data, actions) => {
                                   return actions.order!.capture().then(details => {
                                        toast({ title: "Compra Aprovada!", description: `Obrigado, ${details.payer?.name?.given_name || 'Cliente'}!`});
                                    });
                                }}
                           />
                        </div>
