...
export const twitterFlow = ai.defineFlow(
    ...
    async (input: TwitterMediaInput) => {
        ...
        // 1. Verificar cache primeiro (2 HORAS)

        // 1. Verificar cache permanente primeiro (24h)
        const permanentCached = await loadPermanentCache(username, mediaType, maxResults);
        if (permanentCached) {
            console.log('📦 Retornando dados do cache permanente (arquivo)');
            // Também salvar no cache em memória para acesso mais rápido
            cache.set(cacheKey, { data: permanentCached, timestamp: Date.now() });
            return permanentCached;
        }
        ...
        try {
            ...
            // Salvar no cache em memória e permanente
            cache.set(cacheKey, { data: officialResult, timestamp: Date.now() });
            await savePermanentCache(username, mediaType, maxResults, officialResult);
            ...
            try {
                ...
                if (rapidResult.tweets.length > 0) {
                    ...
                    if (tweetsWithMedia.length === 0 && mediaType === 'photos') {
                        ...
                        if (enhancedResult.tweets.some(t => t.media && t.media.length > 0)) {
                            ...
                            await savePermanentCache(username, mediaType, maxResults, enhancedResult);
                    ...
                    await savePermanentCache(username, mediaType, maxResults, rapidResult);
                ...
                try {
                    ...
                    if (convertedResult.tweets.length > 0) {
                        ...
                        await savePermanentCache(username, mediaType, maxResults, convertedResult);
...
async function fetchFromOfficialAPI(username: string, maxResults: number, mediaType: 'photos' | 'videos' | 'all'): Promise<TwitterMediaOutput> {
    ...
    try {
        ...
        if (!userResponse.ok) {
            ...
            throw new Error(`Erro ao buscar usuário: ${userResponse.status} - ${errorText}`);
        }
        ...
        if (!tweetsResponse.ok) {
            ...
            throw new Error(`Erro ao buscar tweets: ${tweetsResponse.status} - ${errorText}`);
        }
...
async function fetchFromRapidAPI(username: string, maxResults: number, mediaType: 'photos' | 'videos' | 'all'): Promise<TwitterMediaOutput> {
    ...
    try {
        ...
        const url = `https://twitter135.p.rapidapi.com/v2/UserTweetsAndReplies?id=${userId}&count=${maxResults}`;
        ...
        const response = await fetch(
            `https://twitter-api45.p.rapidapi.com/timeline.php?screenname=${username}&count=${maxResults}`,
            { headers }
        );
