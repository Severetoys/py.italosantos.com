...
function getApiUrl() {
  if (typeof window === 'undefined') {
    ...
    return process.env.NEXT_PUBLIC_SITE_URL
      ? `${process.env.NEXT_PUBLIC_SITE_URL}/api/face-auth`
      : 'https://studio-blush-alpha.vercel.app/api/face-auth';
  }
...
const registerFaceIDFlow = ai.defineFlow(
  ...
  async (input): Promise<FaceIDOutput> => {
    try {
      console.log('[Face ID Register] Iniciando cadastro com Firebase...');
      ...
      if (!checkResult.success) {
        console.log('[Face ID Register] Usuário já existe:', checkResult.message);
        return {
          success: false,
...
const loginFaceIDFlow = ai.defineFlow(
  ...
  async (input): Promise<FaceIDOutput> => {
    try {
      console.log('[Face ID Login] Iniciando autenticação com Firebase...');
      
      const result = await verifyUserWithFirebase({
        image: input.image,
        email: input.email
      });
      ...
      if (result.success) {
        ...
        let isVip = false;
        let redirectUrl = '/assinante'; // Sempre direciona para página de assinante
        
        // Verificar status de pagamento se tiver email
        if (result.userEmail) {
          const paymentResult = await checkPaymentStatusFirebase(result.userEmail);
          if (paymentResult.success && paymentResult.isSubscriber) {
            isVip = true;
            // Se é assinante ativo, pode acessar conteúdo exclusivo
